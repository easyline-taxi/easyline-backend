...
class UserSerializerRegister(serializers.HyperlinkedModelSerializer):
    ...
    class Meta:
        ...
        fields = ['name', 'cpf', 'deviceid', 'email', 'city', 'country',
                  'password', 'confirm_password', 'name_of_point', 'admin']
    #
    ...
    def create(self, validated_data):
        ...
        if validated_data.get('admin') and validated_data.get('name_of_point'):
            ...
            point.historic['historic'].append(genAction(user,"Entrou no Ponto","admin"))
            point.employee['employee'].append(genEmployee(user,"admin",point.name))
